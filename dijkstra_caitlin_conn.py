...
def create_map_grid(obstacle_space_color, free_space_color):
    ...
    obstacle_color = obstacle_space_color #(255,0,0) #RED
    # Define obstacle clearance color
    clearance_color = free_space_color #(204,102,0)#(102, 0, 0) #DARK RED #(0,255,0) #GREEN
...
def compute_optimal_path(visited_queue, initial_node_coord, goal_node_coord):
    ...
    first_parent_coord = visited_queue[goal_node_coord][3]
    curr_elem_x = goal_node_coord[0]
    curr_elem_y = goal_node_coord[1]
    
    path_list.append(goal_node_coord)
    ...
    while(not((curr_elem_x == initial_node_coord[0]) and (curr_elem_y == initial_node_coord[1]))):
        for visited_elem in visited_queue:
            curr_elem_x = visited_elem[0]
            curr_elem_y = visited_elem[1]
            curr_coord = (curr_elem_x, curr_elem_y)
            if curr_coord == parent_coord:
                path_list.append(visited_elem)
                parent_coord = visited_queue[visited_elem][3]
                break
...
def dijkstra_approach_alg(obstacle_matrix, c2c_matrix, initial_node_coord, goal_node_coord, map_grid, map_height, map_width):
    ...
    open_dict = {} # keeps track of node queue to be processed
    ...
    if (initial_node_coord[0] == goal_node_coord[0] and initial_node_coord[1] == goal_node_coord[1]): # check if goal node reached
    ...
    while (len(open_dict) != 0): # Stop search when node queue is empty 
        ...
        curr_node_list = open_dict.pop(curr_coord)
        curr_node_coord = (curr_node_x, curr_node_y)

        curr_node = (curr_coord, curr_node_list)
        
        visited_queue[curr_node_coord] = curr_node_list #[0, node_idx, None, (0,0)]
        ...
        # Evaluate children
        curr_parent_idx = curr_node_list[1]
        i = 1
        ...
        while i < 9:
            ...
            if valid_move == True and explored == False:
                ...
                if (is_equal == True): 
                    ...
                    elif (explored == False and is_in_open == True): # Just update C2C and parent 
...
def main(): 
    ...
    # Define RGB colors for map grid image
    explored_color = (255,255,255)#(25,216,184)
    obstacle_space_color = (156,14,38)#(255,255,255)
    ...
    # Resize map image using imutils resize function to optimize processing time
    map_grid = imutils.resize(map_grid, width = 300)
    map_height, map_width, _ = np.shape(map_grid)

    # Set up data structure to keep track of obstacle space
    ...
    # Using different ways to compute the time the dijkstra algorithm takes to solve the given problem space
    start1 = time.time()
    start2 = datetime.now()
    
    visited_queue, goal_found, fig, ax = dijkstra_approach_alg(obstacle_matrix, c2c_matrix, initial_node_coord, goal_node_coord, map_grid, map_height, map_width)    
    end1 = time.time()
    end2 = datetime.now()
    ...
    map_grid = cv2.circle(map_grid, (x_initial,y_initial), radius=0, color=start_node_color, thickness=start_goal_pt_thickness)
    map_grid = cv2.circle(map_grid, (x_goal,y_goal), radius=0, color=goal_node_color, thickness=start_goal_pt_thickness)

    for visited_node in visited_queue:
        
        map_grid[visited_node[1],visited_node[0]] = explored_color 
        map_grid = cv2.circle(map_grid, (x_initial,y_initial), radius=0, color=start_node_color, thickness=start_goal_pt_thickness)
        map_grid = cv2.circle(map_grid, (x_goal,y_goal), radius=0, color=goal_node_color, thickness=start_goal_pt_thickness)
    ...
    for optimal_node in optimal_path:
        map_grid[optimal_node[1],optimal_node[0]] = optimal_path_color
        map_grid = cv2.circle(map_grid, (x_initial,y_initial), radius=0, color=start_node_color, thickness=start_goal_pt_thickness)
        map_grid = cv2.circle(map_grid, (x_goal,y_goal), radius=0, color=goal_node_color, thickness=start_goal_pt_thickness)
