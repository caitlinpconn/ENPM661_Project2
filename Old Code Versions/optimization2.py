...
def dijkstra_approach_alg(obstacle_matrix, c2c_matrix, initial_node_coord, goal_node_coord, map_grid, map_height, map_width):
    ...
    while open_dict: # Stop search when node queue is empty 
        
        debug_counter += 1
        
        curr_node = min(open_dict, key=open_dict.get)
        curr_node1 = open_dict.pop(curr_node)
        ...
        for i in range(1, 9):
            ...
            if valid_move:
                ...
                is_equal = (child_node_x_valid, child_node_y_valid) == goal_node_coord # check if goal node reached
                
                if is_equal: # Goal state found 
                    goal_found = True
                    ...
                    explored = any(elem[0] == child_node_x_valid and elem[1] == child_node_y_valid for elem in visited_queue)
                            
                    is_in_open = child_coord in open_dict
                    
                    #####################################################################################################################

                    if not (explored or is_node_obstacle) and (not is_in_open or cost_to_move < open_dict[child_coord][0]):  
                        node_idx += 1   
                        open_dict[child_coord] = [cost_to_move, node_idx, (curr_node_x,curr_node_y), curr_parent_idx]
...
###################################################################################################### -*- coding: utf-8 -*-
"""
Created on Sun Mar 26 04:03:27 2023
