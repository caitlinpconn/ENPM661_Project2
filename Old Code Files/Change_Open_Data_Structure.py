...
def dijkstra_approach_alg(obstacle_matrix, c2c_matrix, initial_node_coord, goal_node_coord, map_grid, map_height, map_width):
    ...
    while (len(open_dict) != 0): # Stop search when node queue is empty 
        ...
        # Debug statements
        if show_grid == True and debug_counter % 10000 == 0:
        ...
        while i < 9:
            ...
            explored = False
            explored = any(elem == (child_node_x_valid, child_node_y_valid) for elem in visited_queue)
...
def main(): 
    ...
    # Using different ways to compute time dijkstra algorithm takes to solve problem
    start1 = time.time()
    start2 = datetime.now()
    ...
    out = cv2.VideoWriter('conn_dijkstra_alg_video2.avi', cv2.VideoWriter_fourcc(*'XVID'), 50, (map_width,map_height))
