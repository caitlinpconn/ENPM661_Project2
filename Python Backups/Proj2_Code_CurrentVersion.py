...
def dijkstra_approach_alg(obstacle_matrix, c2c_matrix, initial_node_coord, goal_node_coord, map_grid, map_height, map_width):
    ...
    out = cv2.VideoWriter('video3.avi', cv2.VideoWriter_fourcc(*'DIVX'), 2, size) 
    ...
    while (len(open_queue) != 0): # Stop search when node queue is empty 
        ...
        img_array.append(map_grid.copy())
        ...
        while i < 9:
            ...
            if valid_move == True:
                ...
                if (is_equal == True): 
                    ...
                    for e in open_queue:
                        print("Open Queue Current Element: ", e)
                    print()
                    ...
                    # path_list = []
                    # while(curr_elem_x != initial_node_coord[0] and curr_elem_y != initial_node_coord[1]):
                    ...
                    # traversal_list = generate_path(visited_queue)
                    # print(len(traversal_list))
                    # for coord in traversal_list:
                    #     print("Traversal Coordinate: ", coord)
                        
                    # print
                    #     map_grid[coord[1], coord[0]] = (0,0,0)
                    # plt.figure()
                    # im = ax.imshow((map_grid).astype(np.uint8), animated=True, origin="lower")
                    # ims.append([im])
                    ...
                    print("Shape of Image Array: ", np.shape(img_array))
                    print()
                    ...
                    #     fig = plt.figure()
                    #     plt.imshow((img_frame).astype(np.uint8), origin="lower")
                    ...
                    #     img_curr = cv2.imread('file.png')
                    #     img_curr = cv2.resize(img_curr, (width, height) )
...
def main(): 
    ...
    # traversal_list = generate_path(visited_queue)
    # for coord in traversal_list:
    #     map_grid[coord[1], coord[0]] = (0,204,0)
...
####################################
                        ...
                        #formatted_img = cv2.imread(formatted_img)
                        
                        #resized_img = cv2.resize(img_frame, size)
                        #resized_img = cv2.resize(fig_img, size)

                        #output = cv2.cvtColor(resized_img, cv2.COLOR_RGB2BGR)
                        #output = resized_img
                        #output =  np.array(fig_img)
                        # output = image_from_plot
                        # out.write(output)  